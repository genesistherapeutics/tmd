...
from tmd.constants import BOLTZ, DEFAULT_TEMP, KCAL_TO_KJ
from tmd.fe.bar import compute_fwd_and_reverse_df_over_time
from tmd.stats import (
    bootstrap_statistic,
    kendall_tau,
    mean_unsigned_error,
    r_squared,
...
def plot_chiral_restraint_energies(chiral_energies: NDArray, figsize: tuple[float, float] = (13, 10), prefix: str = ""):
    ...
    For use with the outputs of tmd.fe.chiral_utils.make_chiral_flip_heatmaps.
    """
...
def plot_water_proposals_by_state(lambdas: Sequence[float], proposals_by_state: NDArray[np.int32], prefix: str = ""):
    ...
    plt.plot(lambdas, proposals_by_state[:, 0], marker="x")
    plt.xlabel(r"$\lambda$")
...
def plot_retrospective(
    pred_kcal: Sequence[float],
    exp_kcal: Sequence[float],
    title: str,
    output_path: Path | str,
    pred_kcal_errs: Sequence[float] | None = None,
    exp_kcal_errs: Sequence[float] | None = None,
    n_bootstrap: int = 10000,
    is_relative: bool = False,
    pred_label_prefix="Pred",
    exp_label_prefix="Exp",
):
    """Generate a retrospective plot

    Parameters
    ----------
    pred_kcal : sequence of floats
        Predictions, expected to be in units of kcal
    ...
    """
    assert len(pred_kcal) == len(exp_kcal), "Number of predictions and labels must match"
    pred_kcal_ = np.asarray(pred_kcal)
    exp_kcal_ = np.asarray(exp_kcal)
    if pred_kcal_errs is not None:
        assert len(pred_kcal_errs) == len(pred_kcal_)
    if exp_kcal_errs is not None:
        assert len(exp_kcal_errs) == len(exp_kcal_)
    plt.errorbar(exp_kcal_, pred_kcal_, yerr=pred_kcal_errs, xerr=exp_kcal_errs, fmt="o")
    prediction_type = "ΔG"
    if is_relative:
        prediction_type = "ΔΔG"
    ...
    padding = 0.5
    min_axis = np.min(np.concatenate([pred_kcal_, exp_kcal_]))
    max_axis = np.max(np.concatenate([pred_kcal_, exp_kcal_]))
    ...
    def get_ci_interval(statistic) -> str:
        bootstraped_stats = bootstrap_statistic(pred_kcal_, exp_kcal_, statistic, n_bootstrap=n_bootstrap)
