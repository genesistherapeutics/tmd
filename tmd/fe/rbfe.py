...
def _get_default_state_minimization_configs() -> Sequence[minimizer.MinimizationConfig]:
    ...
    Initially attempts to minimize with L-BFGS-B, and falls back to BFGS if it fails.
    L-BFGS-B is twice as fast, but in some edge cases it fails to minimize successfully"""
    return [
        minimizer.ScipyMinimizationConfig(method="L-BFGS-B"),
        minimizer.ScipyMinimizationConfig(method="BFGS", options={"disp": True}),
    ]
...
def setup_optimized_host(
    ...
    mols: list of molecules
        Molecules to optimize the host with

    forcefield: Forcefield
        Forcefield to use

    equilibration_steps: int
        Number of steps to optimize the host with.
    ...
    """
    conf, box = minimizer.pre_equilibrate_host(
        mols, config, forcefield, equilibration_steps=equilibration_steps, seed=seed
    )
    return replace(config, conf=conf, box=box)
...
def setup_in_vacuum(
    ...
    ligand_conf: np.ndarray
        Coordinates of the alchemical ligand

    lamb: float
        Lambda value of the system

    verify_constraints: bool
        If True, verifies the system constraints. This can be slow, so suggested to disable
        if calling this function often.

    Returns
    -------
    5-tuple of coordinates, box, hmr_masses, potentials, None
    """
...
def setup_in_env(
    ...
    run_seed: int
        Seed to use to see the barostat. Adds +1
    ...
    Returns
    -------
    4-tuple of coordinates, hmr_masses, potentials, barostat
    """
    barostat_interval = 25
    system = st.combine_with_host(host.host_system, lamb, host.num_water_atoms, host.omm_topology)
    if verify_constraints:
        assert_default_system_constraints(system)
    host_hmr_masses = model_utils.apply_hmr(host.masses, host.host_system.bond.potential.idxs)
...
def setup_initial_state(
    ...
    # but in a way that should be symmetric for
    # A -> B vs. B -> A edge definitions
    run_seed = int(seed + bytes_to_id(b"".join([p.params.tobytes() for p in potentials]))) % MAX_SEED_VALUE
...
def setup_initial_states(
    ...
    host: HostConfig or None
        Host configuration. If None, return vacuum states.
    ...
    verify_constraints: bool
        Whether or not to verify the constraints of the constructed system. Can be quite a bit slower, suggested
...
def rebalance_lambda_schedule(
    ...
    max_windows: int = DEFAULT_NUM_WINDOWS,
    ...
    if mbar_scalar_overlap < initial_mbar_threshold:
        ...
        if len(greedy_prot) > max_windows:
            warnings.warn(
                f"Optimized schedule has {len(greedy_prot)}, which is more than the max windows allowed: {max_windows}, falling back to initial schedule with {len(initial_lambs)} windows"
            )
            new_schedule = initial_lambs
        else:
...
def optimize_coords_state(
    ...
    if minimization_configs is None:
        minimization_configs = _get_default_state_minimization_configs()
    ...
    for idx, initial_state in enumerate(initial_states):
        ...
        try:
            x_opt = optimize_coords_state(
                ...
                initial_state.box0,
                free_idxs,
                minimization_configs=minimization_configs,
