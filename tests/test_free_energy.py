...
def test_functional(precision):
    ...
    * requesting derivative w.r.t. box causes a runtime error
    ...
    sys_params = [np.array(bp.params.astype(precision)) for bp in bps]
...
@pytest.mark.nocuda
def test_absolute_solvent():
    """Verify the behavior of AbsoluteFreeEnergy in solvent"""
    with path_to_internal_file("tmd.testsystems.fep_benchmark.hif2a", "ligands.sdf") as path_to_ligand:
        mols = utils.read_sdf(path_to_ligand)
    mol = mols[0]
    ...
    for lamb in [0.0, 0.25, 0.5, 1.0]:
        ...
        nb_params = next(p for pot, p in zip(unbound_potentials, sys_params) if isinstance(pot, Nonbonded))
        if lamb == 0.0:
...
@pytest.mark.nocuda
def test_absolute_vacuum_and_solvent_edge_types():
...
def hif2a_ligand_pair_single_topology_lam0_state():
    ...
    forcefield = Forcefield.load_default()
    st = SingleTopology(mol_a, mol_b, core, forcefield)
    state = setup_initial_state(st, 0.0, None, DEFAULT_TEMP, 2023, True)
    return state
...
@pytest.mark.parametrize(
    ("local_steps", "current_iteration", "expected_barostat_step", "expected_water_sampler_step"),
    [
        (4, 0, 0, 0),
        (4, 6, 36, 42),
        (None, 6, 60, 67),
...
@pytest.mark.nocuda
@patch("tmd.fe.free_energy.plot_overlap_summary_figure")
...
def test_get_water_sampler_params(num_windows, max_temperature_scale):
    ...
    for lamb in np.linspace(0.0, 1.0, num_windows, endpoint=True):
        state = setup_initial_state(st, lamb, host_config, DEFAULT_TEMP, 2024, True)
        ...
        system = st.combine_with_host(
            host_config.host_system, lamb, host_config.num_water_atoms, host_config.omm_topology
        )
        ligand_water_params = system.nonbonded_all_pairs.params[host_config.conf.shape[0] :]
        if lamb == 0.0:
        ...
        np.testing.assert_array_equal(
            np.float32(water_sampler_nb_params[host_config.conf.shape[0] :]), np.float32(ligand_water_params)
        )
        np.testing.assert_array_equal(
...
def test_get_water_sampler_params_complex():
    ...
    nb_bp = get_bound_potential_by_type(state.potentials, Nonbonded)
...
def test_compute_potential_matrix(hif2a_ligand_pair_single_topology, n_states: int, max_delta_states: int | None, seed):
    ...
    params_by_state = np.array([bp.params.astype(np.float32) for bp in bps])
...
def test_initial_state_to_bound_impl():
    ...
    solvent_host = setup_optimized_host(solvent_host_config, [st.mol_a, st.mol_b], forcefield)
    initial_state = setup_initial_states(st, solvent_host, DEFAULT_TEMP, [0.5], 2024, verify_constraints=True)[0]
